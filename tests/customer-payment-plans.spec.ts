...
test("Customer payment plans - create and edit", async () => {
  ...
  try {
    const firstSavedCard = page.locator('.rounded-xl.border.bg-card.text-card-foreground.shadow.cursor-pointer').first();
    const cardExists = await firstSavedCard.isVisible({ timeout: 3000 });
    ...
    // Click on the first saved card content area
    await firstSavedCard.locator('.p-4').click();
    
    // Click the radio button inside the first saved card
    await firstSavedCard.getByRole('radio').click();
