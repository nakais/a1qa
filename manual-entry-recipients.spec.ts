...
test('should send document to manually entered recipients', async ({ page }) => {
  ...
  const recipient2 = page.locator('div').filter({ hasText: /^Recipient 2Name \*Email \*Phone \*Note \(Optional\)$/ });
  await recipient2.getByPlaceholder('John Doe').fill('Michael Johnson');
  await recipient2.getByPlaceholder('john@example.com').fill('john@gmail.com');
  await recipient2.getByPlaceholder('+').fill('5553453455');
